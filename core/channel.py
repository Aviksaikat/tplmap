...
class Channel:
    ...
    def _parse_header(self, all_injectable=False):
        ...
        if not "user-agent" in [p.lower() for p in list(self.header_params.keys())]:
    ...
    def _parse_post(self, all_injectable=False):
        ...
        if self.args.get("data"):
            ...
            params_dict_list = urllib.parse.parse_qs(
                self.args.get("data"), keep_blank_values=True
            )

            for param, value_list in list(params_dict_list.items()):
