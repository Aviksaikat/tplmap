class Channel:
    def __init__(
        self, callbacks, configTab, baseRequestResponse, insertionPoint, payloadPosition
    ):
    ...
    def req(self, injection):
        payload = (
            injection
            if self._payloadPosition == "replace"
            else self._insertionPoint.getBaseValue() + injection
        )
        checkRequest = self._insertionPoint.buildRequest(
            self._helpers.stringToBytes(payload)
        )
        checkRequestResponse = self._callbacks.makeHttpRequest(
            self._baseRequestResponse.getHttpService(), checkRequest
        )
        self.messages.append(
            {"injection": injection, "requestResponse": checkRequestResponse}
        )
