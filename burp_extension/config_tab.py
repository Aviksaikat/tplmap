...
from javax.swing import GroupLayout, JCheckBox, JComboBox, JLabel, JPanel
...
class ConfigTab(ITab, JPanel):
    ...
    def __initLayout__(self):
        ...
        for pluginGroup in list(self._plugin_groups.values()):
    ...
    def getTechniques(self):
        return "%s%s" % (
            "R" if self._techRenderedCheckBox.isSelected() else "",
            "T" if self._techTimebasedCheckBox.isSelected() else "",
        )

    def getEngines(self):
        return [
            checkbox.getPlugin()
            for checkbox in self._pluginCheckBoxes
