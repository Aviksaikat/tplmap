...
class ScanIssue(IScanIssue):
    ...
    def getIssueDetail(self):
        ...
        prologue = prologue_template.format(
            parameter=parameter, template=template, language=language
        )

        if self._channel.technique == "render":
            payload = self._channel.messages[self._channel.detect_offset - 1].get(
                "injection"
            )
            ...
            blind_true_payload = self._channel.messages[
                self._channel.detect_offset - 2
            ].get("injection")
            ...
            delta_true_milliseconds = (
                delta_true.seconds * 1000000.0 + delta_true.microseconds
            ) / 1000.0
    ...
    def getHttpMessages(self):
        if self._channel.technique == "render":
            ...
            messages = [
                self._markHttpMessage(
                    detectedMessage.get("requestResponse"),
                    detectedMessage.get("injection"),
                    responseMarkString,
                )
            ]
        elif self._channel.technique == "blind":
            blind_true_message = self._channel.messages[self._channel.detect_offset - 2]
            blind_false_message = self._channel.messages[
                self._channel.detect_offset - 1
            ]
            messages = [
                self._markHttpMessage(
                    blind_true_message.get("requestResponse"),
                    blind_true_message.get("injection"),
                    None,
                ),
                self._markHttpMessage(
                    blind_false_message.get("requestResponse"),
                    blind_false_message.get("injection"),
                    None,
                ),
            ]
        for evaluate in self._channel.messages[self._channel.detect_offset :]:
            messages.append(
                self._markHttpMessage(
                    evaluate.get("requestResponse"), evaluate.get("injection"), None
                )
            )
        return messages
    ...
    def _markHttpMessage(self, requestResponse, injection, responseMarkString):
        ...
        if responseMarkString:
            ...
            start = self._helpers.indexOf(
                response, responseMarkBytes, False, 0, len(response)
            )
            if -1 < start:
                responseMarkers = [array("i", [start, start + len(responseMarkBytes)])]

        requestHighlights = [
            self._insertionPoint.getPayloadOffsets(
                self._helpers.stringToBytes(injection)
            )
        ]
        return self._callbacks.applyMarkers(
            requestResponse, requestHighlights, responseMarkers
        )
